...
export interface Element<PropertiesType extends ElementProperties, OptionsType> {
  transition(
...
export class ImageElement
  ...
  constructor(private package_: Package, private container: Container) {}
  ...
  * transition(
    ...
    const oldValue = oldProperties ? resolveElementValue(oldProperties) : undefined;
    ...
    if (newValue && newValue !== oldValue) {
      const blobUrl = URL.createObjectURL(this.package_.getBlob(newProperties.type, newValue));
    ...
    if (newSprite) {
      for (const [propertyName, propertyValue] of
        Object.entries(newSpriteOldProperties!) as
          Entries<ImageElementResolvedProperties>) {
    ...
    for (const propertyName of propertyNames) {
      ...
      if (oldSpriteChanged) {
        this.transitionProperty(
          oldSprite!, propertyName, oldSpriteNewProperties![propertyName],
          transitionDuration,
        );
      }
      if (newSpriteChanged) {
        this.transitionProperty(
          newSprite!, propertyName, newSpriteNewProperties![propertyName],
          transitionDuration,
        );
      }
    }
